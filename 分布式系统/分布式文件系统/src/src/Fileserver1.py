...
class Servicer(FS_pb2_grpc.FileServerServicer):
    def __init__(self,ip, port,id, password):
        ...
        self.root_path = '../Server_data/File_server_%s/'%(id)  # 服务器的根目录地址
    ...
    def online(self):
        ...
        self.dirstub.fileserver_online(DS_pb2.FileServer_On(Server_ID = self.id, ip = self.ip,port = self.port, password = self.password))
    ...
    def pwd(self,request,context):
        return FS_pb2.PWD_Reply(pwd='server/') # 传一个空串 客户端只知道服务器的相对路径



def run_server(id, ip, port, password):
    servicer = Servicer(ip, port, id, password)
...
if __name__=="__main__":
    run_server(1 , 'localhost', '8083', '111')
    # run_server(2, 'localhost', '8084', '222')
